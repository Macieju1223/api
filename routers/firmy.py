...
def wyswietl_firmy(db: Session = Depends(get_db)):
    firmy = db.query(models.firma).all()
    return firmy

@router.post('/Firmy')
def dodaj_firme(request: schem.firma,db: Session = Depends(get_db)):
...
def wyswietl_firme_poprzez_podanie_numeru(numer,db: Session = Depends(get_db)):
    firma = db.query(models.firma).filter(models.firma.numer == numer).first()
    return firma

@router.delete('/Firma/{id}')
def usuwa_firme_z_bazy(id, db: Session = Depends(get_db)):
    db.query(models.firma).filter(models.firma.id==id).delete(synchronize_session=False)
    db.commit()
    return 'done'

@router.put('/Firma/{id}',)
def edycja_informacji_dotyczacych_firmy(id,request: schem.firma,db: Session = Depends(get_db)):
